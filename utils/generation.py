...
@torch.no_grad()
def generate_audio_train(text, audio_tokens, text_tokens, lang_pr, language='auto', accent='no-accent'):
    global model, codec, vocos, text_tokenizer, text_collater
    text = text.replace("\n", "").strip(" ")
    # detect language
    if language == "auto":
    ...
    # load prompt
    audio_prompts = audio_tokens
    text_prompts = text_tokens
    lang_pr = lang_pr
    lang_pr = code2lang[int(lang_pr)]

    # numpy to tensor
    audio_prompts = audio_prompts.type(torch.int32).to(device)
    text_prompts = text_prompts.type(torch.int32)
